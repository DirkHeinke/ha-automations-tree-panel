import {
  ...
  class AutomationsTree extends LitElement {
        ...
        <div class="automation" style=${"padding-left: " + (i * 25 + 35) + "px"}>
          ...
          <div class="automation-last-run">
            <a href="/config/automation/trace/${automation.id}">
              <span class="automation-trace">${this.fromNow( automation.last_triggered)}</span>
            </a>  
          </div>
        </div>
      `;
     ...
     */
    fromNow(date, nowDate = Date.now(), rft = new Intl.RelativeTimeFormat(undefined, { numeric: "auto" })) {
      if (date == null) {
        return "Never";
      }
      const SECOND = 1000;
